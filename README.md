...
2. 在 `sync_chart` **目录下**执行（或 `-c` 指向配置文件）：
  ```bash
   ...
   python3 sync_chart.py --status                 # 查询最近一次任务
   python3 sync_chart.py --cancel                 # 中断当前任务（插队前先停掉）
   python3 sync_chart.py myapp --title "说明"       # 标题在 [TYPE][name][ver] 后追加说明
  ```
3. 其它参数：`--branch`、`--allow-dirty`、`--token-file`、`--token-env`、`--skip-sync`（跳过 fork 与 upstream 的 fetch/合并，仅用本地克隆当前内容；默认会先同步并校验 PAT）、`--status` / `--cancel` / `--detach` / `--foreground`（批量任务在 Agent 非 TTY 下默认后台，避免调用超时）。**同一时刻只允许一个 job**，已有任务在跑时新提交会失败；插队请先 `--cancel`。帮助：`python3 sync_chart.py -h`。
...
- **Token**：推送与创建 PR 需要 PAT。默认读环境（`GITHUB_TOKEN` / `GH_TOKEN` / `GITHUB_TOKEN_FILE`，或 `--token-env`）。给出 **`--token-file`** 时只读该文件，不读环境中的 token。REST 与 `sync_chart` 一致使用 `Authorization: token`。**勿将** `config.yaml`**、**`github.txt` **提交到远端**。
- **批量列表**：`--batch` 的参数是**文件路径**（txt），不是命令行罗列多个 chart。Agent 批量调用会立刻返回 `job_id`，用 `python3 sync_chart.py --status` 查进度；不要在一次调用里死等整批结束。已有 job 未结束时再次提交会失败；需要插队时先 `python3 sync_chart.py --cancel`。
